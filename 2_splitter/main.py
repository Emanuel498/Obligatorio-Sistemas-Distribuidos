...
def postMeasure(measure: Measure):
    ...
    if(measure.flow < 12.8):
        ...
        response = requests.post("http://producer-alerts/sendAlert", json.dumps(measure.__dict__))
        return JSONResponse(
            status_code=200,
        ...
        response = requests.post("http://producer-data/sendData", json.dumps(measure.__dict__))
        return JSONResponse(
            status_code=200,
